...
class MoveStockSchema(BaseModel):
    ...
    from_box_no: int = Field(
        ..., description="Source box number", json_schema_extra={"example": 7}
    )
    ...
    to_box_no: int = Field(
        ..., description="Destination box number", json_schema_extra={"example": 8}
    )
    to_loc_no: int = Field(
        ...,
...
class LocationSuggestionSchema(BaseModel):
    ...
    box_no: int = Field(
        description="Suggested box number", json_schema_extra={"example": 7}
    )
    loc_no: int = Field(
