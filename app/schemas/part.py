...
class PartResponseSchema(BaseModel):
    ...
    type_id: int | None = Field(
        description="Part type/category ID", json_schema_extra={"example": 1}
    )
    type: TypeResponseSchema | None = Field(
        description="Part type/category details", default=None
    )
    description: str = Field(
    ...
    seller: str | None = Field(
        description="Vendor/supplier name", json_schema_extra={"example": "Digi-Key"}
    )
    seller_link: str | None = Field(
    ...
    def total_quantity(self) -> int:
        ...
        return (
            sum(location.qty for location in self.part_locations)
            if hasattr(self, "part_locations") and self.part_locations
            else 0
        )
...
class PartLocationResponseSchema(BaseModel):
    ...
    id4: str = Field(
        description="4-character part identifier", json_schema_extra={"example": "BZQP"}
    )
    box_no: int = Field(description="Box number", json_schema_extra={"example": 7})
    loc_no: int = Field(
        description="Location number within box", json_schema_extra={"example": 3}
    )
    qty: int = Field(
        description="Quantity at this location", json_schema_extra={"example": 10}
    )
