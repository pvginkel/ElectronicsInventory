...
@api.validate(
    resp=SpectreeResponse(HTTP_200=LocationResponseSchema, HTTP_404=ErrorResponseSchema)
)
@handle_api_errors
def get_location_details(box_no: int, loc_no: int):
    """Get specific location details."""
    stmt = select(Location).where(Location.box_no == box_no, Location.loc_no == loc_no)
