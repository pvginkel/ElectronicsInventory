...
class TestCapacityValidation:
    ...
    def test_box_service_creates_correct_number_of_locations(self, app: Flask):
        ...
        with app.app_context():
            ...
            for i, capacity in enumerate(capacities, 1):
                box = BoxService.create_box(f"Box {i}", capacity)
    ...
    def test_box_service_location_numbering_sequential(self, app: Flask):
        """Test that locations are numbered sequentially from 1 to capacity."""
        with app.app_context():
            capacity = 15
            box = BoxService.create_box("Test Box", capacity)
    ...
    def test_capacity_increase_creates_new_locations(self, app: Flask):
        ...
        with app.app_context():
            ...
            # Increase capacity
            updated_box = BoxService.update_box_capacity(original_box_no, 10, "Updated Box")
            ...
            # Verify in database
            db_locations = db.session.query(Location).filter_by(box_no=original_box_no).all()
    ...
    def test_capacity_decrease_removes_higher_numbered_locations(self, app: Flask):
        ...
        with app.app_context():
            ...
            # Decrease capacity
            updated_box = BoxService.update_box_capacity(original_box_no, 6, "Smaller Box")
            ...
            high_locations = db.session.query(Location).filter(
                Location.box_no == original_box_no,
                Location.loc_no > 6
            ).all()
            assert len(high_locations) == 0

    def test_capacity_same_value_no_location_changes(self, app: Flask):
        ...
        with app.app_context():
            ...
            updated_box = BoxService.update_box_capacity(original_box_no, 8, "Updated Description")
    ...
    def test_minimum_capacity_edge_case(self, app: Flask):
        """Test creating box with minimum capacity (1)."""
        with app.app_context():
            box = BoxService.create_box("Single Slot Box", 1)

            assert box.capacity == 1
            assert len(box.locations) == 1
            assert box.locations[0].loc_no == 1

    def test_large_capacity_handling(self, app: Flask):
        """Test creating box with large capacity."""
        with app.app_context():
            large_capacity = 500
            box = BoxService.create_box("Large Box", large_capacity)
    ...
    def test_capacity_decrease_to_minimum(self, app: Flask):
        """Test decreasing capacity to minimum (1)."""
        with app.app_context():
            # Create box with larger capacity
            box = BoxService.create_box("Test Box", 20)
            original_box_no = box.box_no

            # Decrease to minimum
            updated_box = BoxService.update_box_capacity(original_box_no, 1, "Minimal Box")
    ...
    def test_multiple_capacity_changes(self, app: Flask):
        ...
        with app.app_context():
            ...
            # First increase
            updated_box = BoxService.update_box_capacity(box_no, 15, "Expanded Box")
            assert updated_box is not None
            assert len(updated_box.locations) == 15

            # Then decrease
            updated_box = BoxService.update_box_capacity(box_no, 8, "Shrunk Box")
            assert updated_box is not None
            assert len(updated_box.locations) == 8

            # Then increase again
            updated_box = BoxService.update_box_capacity(box_no, 12, "Re-expanded Box")
            assert updated_box is not None
    ...
    def test_capacity_validation_in_response_schema(self, app: Flask):
        """Test that BoxResponseSchema properly handles capacity."""
        with app.app_context():
            # Create box through service
            box = BoxService.create_box("Test Box", 7)
    ...
    def test_location_generation_atomicity(self, app: Flask):
    ...
    def test_capacity_boundary_conditions(self, app: Flask):
        ...
        with app.app_context():
            ...
            for i, capacity in enumerate(powers_of_2):
                box = BoxService.create_box(f"Box 2^{i}", capacity)
                assert len(box.locations) == capacity
