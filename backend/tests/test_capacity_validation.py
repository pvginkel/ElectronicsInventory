...
class TestCapacityValidation:
    ...
    def test_box_service_creates_correct_number_of_locations(
        ...
        with app.app_context():
            ...
            for i, capacity in enumerate(capacities, 1):
                box = container.box_service().create_box(f"Box {i}", capacity)
    ...
    def test_box_service_location_numbering_sequential(
        ...
        with app.app_context():
            capacity = 15
            box = container.box_service().create_box("Test Box", capacity)
    ...
    def test_capacity_increase_creates_new_locations(
        ...
        with app.app_context():
            ...
            # Increase capacity
            updated_box = container.box_service().update_box_capacity(
                original_box_no, 10, "Updated Box"
            )
    ...
    def test_capacity_same_value_no_location_changes(
        ...
        with app.app_context():
            ...
            updated_box = container.box_service().update_box_capacity(
                original_box_no, 8, "Updated Description"
            )
    ...
    def test_minimum_capacity_edge_case(self, app: Flask, session: Session, container: ServiceContainer):
        """Test creating box with minimum capacity (1)."""
        with app.app_context():
            box = container.box_service().create_box("Single Slot Box", 1)

            assert box.capacity == 1
            assert len(box.locations) == 1
            assert box.locations[0].loc_no == 1

    def test_large_capacity_handling(self, app: Flask, session: Session, container: ServiceContainer):
        """Test creating box with large capacity."""
        with app.app_context():
            large_capacity = 500
            box = container.box_service().create_box("Large Box", large_capacity)
    ...
    def test_capacity_decrease_to_minimum(self, app: Flask, session: Session, container: ServiceContainer):
    ...
    def test_multiple_capacity_changes(self, app: Flask, session: Session, container: ServiceContainer):
    ...
    def test_capacity_validation_in_response_schema(self, app: Flask, session: Session, container: ServiceContainer):
        """Test that service returns ORM box models properly."""
        with app.app_context():
            # Create box through service
            box = container.box_service().create_box("Test Box", 7)
    ...
    def test_location_generation_atomicity(self, app: Flask, session: Session, container: ServiceContainer):
    ...
    def test_capacity_boundary_conditions(self, app: Flask, session: Session, container: ServiceContainer):
        ...
        with app.app_context():
            ...
            for i, capacity in enumerate(powers_of_2):
                box = container.box_service().create_box(f"Box 2^{i}", capacity)
                assert len(box.locations) == capacity
