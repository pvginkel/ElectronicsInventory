...
def _build_service(
    ...
    Args:
        settings: Application settings (should have oidc_enabled=True for most tests)
        discovery_doc: OIDC discovery document to return from the mock.
            If None and OIDC is enabled, a valid default discovery doc is used.

    Returns:
        OidcClientService instance
    """
...
class TestOidcEndpointDiscovery:
    ...
    def test_discovery_without_end_session_endpoint(self, oidc_settings: Settings) -> None:
        """Test that end_session_endpoint is optional and stored as None when absent."""
        discovery_doc = {
            "authorization_endpoint": "https://auth.example.com/auth",
            "token_endpoint": "https://auth.example.com/token",
            "jwks_uri": "https://auth.example.com/certs",
...
class TestPkceChallenge:
    ...
    def test_generate_pkce_challenge_s256(self, service: OidcClientService) -> None:
        ...
        The challenge must be: BASE64URL(SHA256(code_verifier)), without padding.
        """
        code_verifier = "dBjftJeZ4CVP-mB92K27uhbUJU1p1r_wW1gFWFOEjXk"

        # Compute expected challenge manually per RFC 7636
        sha256_digest = hashlib.sha256(code_verifier.encode("ascii")).digest()
        expected_challenge = base64.urlsafe_b64encode(sha256_digest).rstrip(b"=").decode("ascii")
    ...
    def test_pkce_challenge_different_verifiers_differ(self, service: OidcClientService) -> None:
...
class TestGenerateAuthorizationUrl:
    ...
    def test_authorization_url_contains_required_params(self, service: OidcClientService) -> None:
        """Test that the generated authorization URL contains all required OAuth2/PKCE parameters."""
        url, auth_state = service.generate_authorization_url("http://localhost:3000/dashboard")

        parsed = urlparse(url)
        params = parse_qs(parsed.query)

        assert params["client_id"] == ["ei-backend"]
        ...
        # State must match the nonce in auth_state
    ...
    def test_authorization_url_base(self, service: OidcClientService) -> None:
        ...
        assert base_url == "https://auth.example.com/realms/ei/protocol/openid-connect/auth"

    def test_auth_state_contains_pkce_verifier(self, service: OidcClientService) -> None:
    ...
    def test_auth_state_contains_redirect_url(self, service: OidcClientService) -> None:
        """Test that the returned AuthState preserves the original redirect URL."""
        redirect = "http://localhost:3000/parts/ABCD"
    ...
    def test_pkce_challenge_matches_verifier(self, service: OidcClientService) -> None:
        ...
        # Compute expected challenge from the verifier in auth_state
...
class TestExchangeCodeForTokens:
    ...
    def test_exchange_sends_correct_request(self, service: OidcClientService) -> None:
        ...
        assert call_args.args[0] == "https://auth.example.com/realms/ei/protocol/openid-connect/token"
        ...
        assert posted_data["client_id"] == "ei-backend"
    ...
    def test_exchange_http_error_raises_authentication_exception(self, service: OidcClientService) -> None:
        ...
        with patch("httpx.post") as mock_post:
            ...
            with pytest.raises(AuthenticationException, match="Failed to exchange authorization code"):
    ...
    def test_exchange_defaults_token_type_and_expires(self, service: OidcClientService) -> None:
        ...
        token_data = {
            "access_token": "eyJ-token",
            # token_type and expires_in intentionally omitted
        }
    ...
    def test_exchange_optional_fields_none_when_absent(self, service: OidcClientService) -> None:
...
class TestRefreshAccessToken:
    ...
    def test_refresh_success(self, service: OidcClientService) -> None:
        ...
        assert result.id_token == "eyJ-new-id-token"
        assert result.refresh_token == "eyJ-new-refresh-token"
        assert result.token_type == "Bearer"
        assert result.expires_in == 300

    def test_refresh_sends_correct_request(self, service: OidcClientService) -> None:
        """Test that refresh sends the correct form data with refresh_token grant type."""
        token_data = {
            "access_token": "eyJ-token",
            "token_type": "Bearer",
            "expires_in": 300,
    ...
    def test_refresh_http_error_raises_authentication_exception(self, service: OidcClientService) -> None:
        """Test that an HTTP error during refresh raises AuthenticationException."""
        with patch("httpx.post") as mock_post:
            mock_post.side_effect = httpx.ConnectError("Connection refused")

            with pytest.raises(AuthenticationException, match="Failed to refresh access token"):
                service.refresh_access_token("refresh-token")
    ...
    def test_refresh_preserves_original_refresh_token_when_not_rotated(
        ...
        token_data = {
            ...
            "token_type": "Bearer",
            "expires_in": 300,
            # No refresh_token in response -- provider did not rotate it
        }
        ...
        # Should fall back to the original refresh token
...
class TestOidcDisabled:
    ...
    def test_endpoints_property_raises_when_oidc_disabled(self, test_settings: Settings) -> None:
        """Test that accessing endpoints raises ValueError when OIDC is disabled."""
        service = _build_service(test_settings)  # oidc_enabled=False by default
    ...
    def test_endpoints_internal_state_is_none_when_disabled(self, test_settings: Settings) -> None:
