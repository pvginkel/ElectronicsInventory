...
class TestImageService:
    ...
    def test_process_uploaded_image_needs_resize(self, image_service, large_image_bytes):
        """Test processing a large image that needs resizing."""
        file_data = io.BytesIO(large_image_bytes)
        
        result_data, metadata = image_service.process_uploaded_image(file_data)
        
        # Image should be resized down
        result_data.seek(0)
        processed_img = Image.open(result_data)
        assert max(processed_img.size) <= 1920  # Should be resized to max 1920px
    ...
    def test_process_uploaded_image_invalid_format(self, image_service):
        ...
        assert "process uploaded image" in str(exc_info.value)
    ...
    def test_generate_thumbnail_creates_file(self, image_service, sample_image_bytes, temp_dir):
        ...
        attachment_id = 123
        original_data = io.BytesIO(sample_image_bytes)
        size = 150
        
        thumbnail_path = image_service.generate_thumbnail(attachment_id, original_data, size)
    ...
    def test_generate_thumbnail_different_sizes(self, image_service, sample_image_bytes, temp_dir):
        ...
        path_150 = image_service.generate_thumbnail(attachment_id, original_data, 150)
        path_300 = image_service.generate_thumbnail(attachment_id, original_data, 300)
    ...
    def test_get_thumbnail_path_creates_if_not_exists(self, image_service, temp_dir):
        ...
        with patch.object(image_service, 's3_service', mock_s3_service):
            thumbnail_path = image_service.get_thumbnail_path(attachment_id, s3_key, size)
    ...
    def test_get_thumbnail_path_returns_existing(self, image_service, sample_image_bytes, temp_dir):
        ...
        original_data = io.BytesIO(sample_image_bytes)
        expected_path = image_service.generate_thumbnail(attachment_id, original_data, size)
    ...
    def test_thumbnail_path_structure(self, image_service):
        """Test thumbnail path structure is consistent."""
        attachment_id = 123
        size = 150
        
        path = image_service._get_thumbnail_path(attachment_id, size)
        
        # Should be in format: {base_path}/attachment_{id}_{size}.jpg
        expected = image_service.thumbnail_base_path / f"attachment_{attachment_id}_{size}.jpg"
    ...
    def test_generate_thumbnail_handles_portrait_orientation(self, image_service):
        ...
        attachment_id = 111
        thumbnail_path = image_service.generate_thumbnail(attachment_id, portrait_bytes, 150)
    ...
    def test_generate_thumbnail_handles_landscape_orientation(self, image_service):
        ...
        attachment_id = 222
        thumbnail_path = image_service.generate_thumbnail(attachment_id, landscape_bytes, 150)
