...
class TestAuthenticationMiddleware:
    ...
    def test_bearer_token_authentication(
        ...
        response = client.get(
            "/api/parts", headers={"Authorization": f"Bearer {token}"}
        )
    ...
    def test_any_authenticated_user_has_access(
        ...
        """Test that any authenticated user can access endpoints (EI default: no role check)."""
        client = oidc_app.test_client()

        # Token with a non-admin role should still pass (authenticated-only default)
    ...
    def test_public_login_endpoint_accessible_without_token(
        ...
        # Login endpoint is public but requires 'redirect' parameter
    ...
    def test_oidc_disabled_bypasses_authentication(self, client: Any):
        ...
        # Standard client fixture has oidc_enabled=False
        response = client.get("/api/parts")
        assert response.status_code == 200
...
class TestTokenRefreshMiddleware:
    ...
    def test_expired_access_token_with_valid_refresh_sets_new_cookies(
        ...
        new_refresh_token = jwt.encode(
            {"sub": "test-user", "exp": int(time.time()) + 86400, "typ": "Refresh"},
            generate_test_jwt.private_key,
            algorithm="RS256",
        )
