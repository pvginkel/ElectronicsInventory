...
def _create_auth_service(
    ...
    Args:
        auth_settings: Settings with OIDC enabled
        mock_oidc_discovery: Mock discovery document
        generate_test_jwt: Test JWT generator fixture (with public_key attribute)
...
class TestAuthService:
    ...
    def test_validate_token_invalid_issuer(
        ...
        assert "issuer" in str(exc_info.value).lower() or "audience" in str(exc_info.value).lower()
